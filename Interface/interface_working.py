...
def rand_sentence_generator():
    ...
    relativizers = {'person':['who','that'],'animate':['that','which'],'inanimate':['that','which']}
...
@post('/login')
def checkLogin():
    global username, points, userdict
    ...
    if username in list(userdict.keys()):
        # print("already here")
        pass     ### we don't know if we need to do anything here yet

    else:
        userdict[username] = [my_multiple_sentences,my_good_sentence]     ### this puts an empty value to the key for a newly enter name
        # print('wasnt here')
    print(userdict[username][0])
...
def newpage():
    ...
    if user_combo == str(my_good_sentence).lower().translate(str.maketrans('','',string.punctuation)) \
            ...
            or re.sub('who','that',user_combo) == str(my_good_sentence).lower().translate(str.maketrans('','',string.punctuation)) \
            or re.sub('that','who',user_combo) == str(my_good_sentence).lower().translate(str.maketrans('','',string.punctuation)):
        points = points + 1
        passfail = 'True'
